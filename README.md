...
# The _AIRISC_ RISC-V Processor for Embedded AI
...
a customizable yet powerful system on chip.
...
[RISC-V user and privileged specifications](https://riscv.org/technical/specifications/). The CPU implements a 
Harvard architecture with a four-level pipeline and separate AHB-Lite interfaces for the instruction fetch
...
a **SPI** controller also with configurable FIFO buffers, general-purpose IO ports (**GPIO**) and a RISC-V-compatible
...
FPGA board and can ported easily to other boards, platforms and technologies as the core itself is written in
...
[AIRISC fact sheet (pdf)](https://www.ims.fraunhofer.de/content/dam/ims/en/documents/2022-10-20%20Info%20Sheet_AIRISC%20-%20Family%20of%20RISC-V%20Cores_DINA4_CMYK_web.pdf)
...
fast and easy development of on-the-edge AI applications like inference of neuronal networks. In additions to
...
This section provides instructions how to

1. synthesize the core complex for a specific FPGA board.
2. run the pre-compiled example application program using openOCD and GDB.
3. run a simulation using the default testbench, which checks the RISC-V ISA as well as the general core functionality.
...
- OpenOCD: [risc-v openOCD](https://github.com/riscv/riscv-openocd) 0.10.0+dev-01259-gfb477376d; or upstream openOCD
