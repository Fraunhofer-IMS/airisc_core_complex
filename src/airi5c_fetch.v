...
// Last Modified    : 15.12.2022 [nolting]
// Version          : 2.0
...
//                    15.12.22 - complete rework; general concept/code adapted from github.com/stnolting/neorv32/blob/main/rtl/core/neorv32_cpu_control.vhd
//
...
assign ipb_we[2] = ((state_r == StFetch) && (state_prev_r != StWait) &&                      (imem_hready_i) && (ipb_free_global)) ? 1'b1 : 1'b0; // status


// --------------------------------------------------------------------------------------------
// Instruction prefetch buffer
// built from three individual FIFOs to handle instruction:
// - instruction low half-word
// - instruction high half-word
// - instruction address
// -> required for handling unaligned 32-bit instruction (C ISA extension only) and for relaxing
// instruction fetch memory traffic / access times
// --------------------------------------------------------------------------------------------

airi5c_prebuf_fifo #( // low half-word of instruction word
  .FIFO_DEPTH(`IPB_DEPTH),
  .FIFO_WIDTH(`XPR_LEN/2)
) ipb_lo (
  .clk_i(clk_i),
  .rstn_i(rst_ni),
  .clear_i(ipb_clear),
  .hfull_o(ipb_hfull[0]),
  .we_i(ipb_we[0]),
  .data_i(imem_hrdata_i[(`XPR_LEN/2)-1:0]),
  .free_o(ipb_free[0]),
  .re_i(ipb_re[0]),
  .data_o(ipb_cmd_lo),
  .avail_o(ipb_avail[0])
...
airi5c_prebuf_fifo #( // high half-word of instruction word
  .FIFO_DEPTH(`IPB_DEPTH),
  .FIFO_WIDTH(`XPR_LEN/2)
) ipb_hi (
  .clk_i(clk_i),
  ...
  .hfull_o(ipb_hfull[1]),
  .we_i(ipb_we[1]),
  .data_i(imem_hrdata_i[`XPR_LEN-1:`XPR_LEN/2]),
  .free_o(ipb_free[1]),
  .re_i(ipb_re[1]),
  .data_o(ipb_cmd_hi),
  .avail_o(ipb_avail[1])
...
airi5c_prebuf_fifo #( // error/status bits
  .FIFO_DEPTH(`IPB_DEPTH),
  .FIFO_WIDTH(1)
) ipb_status (
  .clk_i(clk_i),
...
`ifdef ISA_EXT_C // -------- intermix of 32-bit and 16-bit instructions -------- //
  ...
  assign ipb_re[2] = ipb_re[0] | ipb_re[1];       // status FIFO
...
`else // -------- 32-bit instructions only -------- //
  ...
  // interface to IPB
  assign ipb_re = ((de_ready_i) && (&ipb_avail)) ? 3'b111 : 3'b000;
...
assign error_in_if_o = ipb_err;


wire [1:0] test_debug = issue_valid;
...
assign predicted_branch_if_o = 1'b0; // will be implemented for next release
